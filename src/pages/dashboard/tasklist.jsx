...
import { ArrowUpTrayIcon, InformationCircleIcon } from "@heroicons/react/24/solid";
...
const TABLE_HEAD = ["Actions", "Detail", "Kode", "Title", "Task Severity", "Due Date", "Project Manager", "Business Analyst", "Assignee", "Created By", "Updated By"];
...
export function TaskList() {
  ...
  const [formData, setFormData] = useState({ kode: "", title: "", description: "", attachment: "", duedate: "", project_id: "", assignee: "", business_analyst: "", project_manager: "", task_severity: "" });
  ...
  const fetchProjectTask = async () => {
    ...
    try {
      ...
      if (responseData && Array.isArray(responseData.data)) {
        setTABLE_ROWS(responseData.data.length === 0 ? [] : responseData.data);
        console.log("Project Task Data: ", responseData.data);
      } else {
        console.error("Received data is not an array or missing 'data' property: ", responseData);
        setTABLE_ROWS([]);
      }
  ...
  const filterRows = (rows) => {
    ...
    return rows.filter(row => {
      ...
      return ['kode', 'title', 'description', 'task_severity', 'duedate', 'assignee', 'created_by_name', 'updated_by_name', 'business_analyst'].some(key =>
  ...
  return (
    <>
      ...
      <Dialog open={isOpenSub} handler={() => {}} dismiss={{ outsidePointerDown: false, escapeKeyDown: false }} size="sm">
        ...
        <DialogBody divider className="max-h-96 overflow-y-auto">
          <div className="space-y-6 font-poppins">
            ...
            >
              {getUsernamesForProject(formData.project_id).map(assignee_name => (
      ...
      <Dialog open={isOpen} handler={() => {}} dismiss={{ outsidePointerDown: false, escapeKeyDown: false }} size="sm">
        <DialogHeader className="font-poppins text-xl font-semibold">{isEditing ? "Edit Project Task" : "Add Project Task"}</DialogHeader>
        <DialogBody divider className="max-h-96 overflow-y-auto">
          <div className="space-y-6 font-poppins">
          <Input
              label="Task Name"
            ...
            <div className="space-y-6 font-poppins">
              ...
              >
                <Option value="HIGH" className="font-poppins">High</Option>
                <Option value="MEDIUM" className="font-poppins">Medium</Option>
                <Option value="LOW" className="font-poppins">Low</Option>
              </MaterialSelect>
            </div>
            <div className="w-full mx-auto">
              <label className="block text-xs font-normal mb-2">Attachment</label>
